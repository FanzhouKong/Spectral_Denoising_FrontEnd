...
# Save the molecule to an SVG file
    if savepath.endswith('.svg') == False:
        savepath = savepath + '.svg'
    Draw.MolToFile(mol, savepath, size=(300, 300), imageType='svg')
def mol_to_cdf(mol, savepath):
    AllChem.EmbedMolecule(mol)      
    # Generate 3D coordinates

    # Extract atomic numbers and 3D coordinates
    atomic_numbers = [atom.GetAtomicNum() for atom in mol.GetAtoms()]
    conf = mol.GetConformer()
    coords = np.array([list(conf.GetAtomPosition(i)) for i in range(mol.GetNumAtoms())])
    if savepath.endswith('.cdf') == False:
        savepath = savepath + '.cdf'
    # Save data to a CDF file
    with Dataset(savepath, "w", format="NETCDF4") as cdf:
        # Create dimensions
        num_atoms = cdf.createDimension("num_atoms", len(atomic_numbers))
        coords_dim = cdf.createDimension("coords", 3)
        
        # Create variables
        atomic_numbers_var = cdf.createVariable("atomic_numbers", "i4", ("num_atoms",))
        coords_var = cdf.createVariable("coordinates", "f4", ("num_atoms", "coords"))
        
        # Assign data to variables
        atomic_numbers_var[:] = atomic_numbers
        coords_var[:, :] = coords
