...
def get_n_workers(reserve=2, min_workers=1):
    total = mp.cpu_count()
    return max(min_workers, total - reserve)
def spectral_denoising_batch(msms_query, smiles_query, adduct_query, mass_tolerance = 0.005):
    ...
    """
    # print('i am in neww')
    num_workers = get_n_workers(reserve=2, min_workers=1)
    ...
    with mp.Pool(processes=6) as pool:
        ...
        # results = pool.starmap(spectral_denoising, [(msms, smiles, adduct, mass_tolerance) for msms, smiles, adduct 
...
def spectral_denoising_with_master_formulas(msms, master_formula, benzene_tag, query_pmz,mass_tolerance = 0.005):
    ...
    mass_threshold=real_mass_threshold
...
def formula_denoising(msms, smiles, adduct, mass_tolerance=0.005):
    ...
    frag_msms_denoised = frag_msms[tag]
    return so.add_spectra(frag_msms_denoised, pmz_msms)
def prep_formula(smiles, adduct):
    ...
    return(re.sub(r'[\[\]\+\-]', '', master_formula.formula))
...
def get_pmz_statistics(msms, c_pmz, mass_tolerance):
    ...
    if r_deviation > mass_tolerance:
        if r_deviation*1.75>2*mass_tolerance:
            r_deviation = 2*mass_tolerance
        else:
            r_deviation = r_deviation * 1.75
    else:
        r_deviation = mass_tolerance
    return r_pmz, r_deviation
